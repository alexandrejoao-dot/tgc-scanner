...
export async function pesquisarMagic(query, setCode) {
  let q = `name:${query}`;
...
// Lista de expansões, para o filtro de pesquisa — exclui sets só digitais
// (Arena/MTGO), que não interessam para uma coleção física.
let cacheSetsMagic = null;
export async function listarSetsMagic() {
  if (cacheSetsMagic) return cacheSetsMagic;
  const resp = await fetch(`${BASE}/sets`, { headers: HEADERS });
  if (!resp.ok) throw new Error(`Scryfall respondeu ${resp.status}`);
  const dados = await resp.json();
  cacheSetsMagic = (dados.data || [])
    .filter((s) => !s.digital)
    .map((s) => ({
      id: s.code,
      nome: s.name,
      ano: s.released_at ? Number(s.released_at.slice(0, 4)) : null,
    }));
  return cacheSetsMagic;
}
